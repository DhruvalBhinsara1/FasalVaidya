-- =================================================================
-- FIX: Update Batch Sync Functions to Use Device-Bound Auth
-- =================================================================
-- Problem: RPC functions were creating duplicate users with device_fingerprint
-- Solution: Use device_id from upsert_device_user instead
-- Run this in Supabase SQL Editor to fix the duplicate user issue
-- =================================================================

-- =================================================================
-- FUNCTION: Batch Upsert Leaf Scans (DEVICE-BOUND VERSION)
-- =================================================================
...
-- =================================================================
-- FUNCTION: Batch Upsert Diagnoses (DEVICE-BOUND VERSION)
-- =================================================================
...
BEGIN
    ...
    IF current_user_id IS NULL THEN
        RETURN QUERY SELECT 
            FALSE, 
            0::INTEGER, 
            jsonb_array_length(diagnoses_data)::INTEGER,
            jsonb_build_array(jsonb_build_object(
                'error', 'User not found. Call upsert_device_user first.'
            ));
        RETURN;
...
-- =================================================================
-- FUNCTION: Batch Upsert Recommendations (DEVICE-BOUND VERSION)
-- =================================================================
...
BEGIN
    ...
    IF current_user_id IS NULL THEN
        RETURN QUERY SELECT 
            FALSE, 
            0::INTEGER, 
            jsonb_array_length(recommendations_data)::INTEGER,
            jsonb_build_array(jsonb_build_object(
                'error', 'User not found. Call upsert_device_user first.'
            ));
        RETURN;
...
-- =================================================================
-- FUNCTION: Get Changes Since Last Sync (DEVICE-BOUND VERSION)
-- =================================================================
...
BEGIN
    ...
    -- If still not found, return error
    IF current_user_id IS NULL THEN
        RETURN jsonb_build_object('error', 'User not found. Call upsert_device_user first.');
    END IF;
...
-- GRANT PERMISSIONS
-- =================================================================
GRANT EXECUTE ON FUNCTION public.sync_leaf_scans_batch(JSONB, UUID) TO authenticated;
GRANT EXECUTE ON FUNCTION public.sync_diagnoses_batch(JSONB, UUID) TO authenticated;
GRANT EXECUTE ON FUNCTION public.sync_recommendations_batch(JSONB, UUID) TO authenticated;
GRANT EXECUTE ON FUNCTION public.get_changes_since(TEXT, TIMESTAMPTZ, UUID) TO authenticated;

-- =================================================================
-- CLEANUP: Delete duplicate users with device_fingerprint
-- =================================================================
-- WARNING: Only run this after confirming you have device_id users
-- This will delete the old sync-client users
...
-- If you're satisfied, uncomment this to delete:
